...
export default function PricingCards({ onPlanSelect, userType, compact }) {
  ...
  const handleSelect = (tierKey, billingCycle) => {
    if (onPlanSelect) {
      onPlanSelect(tierKey, billingCycle);
    } else {
          ...
          return (
            <div key={idx} className={`card pricing-card ${tier.popular ? 'popular' : ''} ${isRecommended ? 'recommended' : ''}`}>
              ...
              <button
                ...
                onClick={() => handleSelect(tierKey, isYearly ? 'yearly' : 'monthly')}
              >
                {onPlanSelect ? 'Choose Plan' : 'Get Started'}
              </button>
            </div>
          );
        })}
